...
import logging
import queue
from core.config_loader import check_discovery, load_agent_configs
...
from core_msgs.instance_aggregate.instantiate_handshake import InstantiateInitiator, InstantiateEnvelope, \
    InstantiateAction
from core_msgs.instance_aggregate.mission_handshake import MissionSession,  MissionEnvelope
...
class AggregateTwin(MessageDispatcher):
    ...
    def __init__(
        ...
        self.agent_configs = load_agent_configs(AGENT_CONFIG_PATH)
    ...
    def _handle_discovery(self, msg: DiscoveryMessage) -> None:
        ...
        if isinstance(result, DiscoveryMessage):
            self._pending_discovery[agent_name] = result
            self._request_instance(agent_name, result)
        else: # else dict for gui to check
            self.discoveries_gui.update(result)
